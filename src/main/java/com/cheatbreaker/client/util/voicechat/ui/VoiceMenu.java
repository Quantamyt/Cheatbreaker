package com.cheatbreaker.client.util.voicechat.ui;
...
public class VoiceMenu extends AbstractGui {
    ...
    public void initGui() {
        ...
        if (cheatbreaker.getCBNetHandler().isVoiceChatEnabled() && cheatbreaker.getCBNetHandler().getVoiceChannels() != null) {
            ...
            boolean var1 = cheatbreaker.getCBNetHandler().getPlayersInVoiceChannel().contains(this.mc.thePlayer.getGameProfile().getId());
    ...
    public void drawMenu(float var1, float var2) {
        ...
        if (cheatbreaker.getCBNetHandler().isVoiceChatEnabled() && cheatbreaker.getCBNetHandler().getVoiceChannels() != null) {
            ...
            VoiceMenu.cheatbreaker.playBold18px.drawString("VOICE CHAT", var3, var5 - 4.0f, -1);
            ...
            VoiceMenu.cheatbreaker.playBold18px.drawString("VOICE CHAT IS NOT SUPPORTED", var3, var4, -1);
        }
    }
    ...
    protected void mouseClicked(float var1, float var2, int var3) {
        if (this.voiceChannelButtons != null) {
            ...
            while (true) {
                ...
                if (!var4.hasNext()) {
                    if (this.currentVoiceChannel != null) {
                        ...
                        if (this.deafenButton.isMouseInside(var1, var2)) {
                            ...
                            if (!cheatbreaker.getCBNetHandler().getPlayersInVoiceChannel().removeIf(var1x -> var1x.equals(var9))) {
                                cheatbreaker.getCBNetHandler().getPlayersInVoiceChannel().add(var9);
                            }
                            this.deafenButton.setString(cheatbreaker.getCBNetHandler().getPlayersInVoiceChannel().contains(this.mc.thePlayer.getGameProfile().getId()) ? "Un-deafen" : "Deafen");
                        }
                ...
                button = var4.next();
                if (!button.isMouseInside(var1, var2) || this.currentVoiceChannel == (var6 = this.getVoiceChannel(button.getString())))
                    continue;
                for (GradientTextButton var8 : this.voiceChannelButtons) {
                    if (this.currentVoiceChannel == cheatbreaker.getCBNetHandler().getCurrentVoiceChannel() || !var8.getString().equals(this.currentVoiceChannel.getName()))
                        continue;
                    var8.buttonColor3();
                }
                this.mc.getSoundHandler().playSound(PositionedSoundRecord.create(new ResourceLocation("gui.button.press"), 1.0f));
    ...
    private void drawCurrentChannel(float var1, float var2, float var3, float var4) {
        ...
        VoiceMenu.cheatbreaker.playBold18px.drawString(this.currentVoiceChannel.getName(), var3, (var4 -= var6 / 2.0f) - 14.0f, -1);
        ...
        for (VoiceUser users : listeners) {
            ...
            boolean isUser = cheatbreaker.getCBNetHandler().getPlayersInVoiceChannel().contains(users.getPlayerUUID());
            ...
            VoiceMenu.cheatbreaker.ubuntuMedium16px.drawString(users.getPlayerName().toUpperCase(), var14 + 6.0f, var13 + 2.0f, listener ? -1 : 0x6FFFFFFF);
            ++userIndex;
        }
    }
    ...
    private void handleMouseClick(float var1, float var2, float var3, float var4) {
        ...
        for (VoiceUser listener : this.currentVoiceChannel.getUsers()) {
            ...
            if (!listener.getPlayerUUID().equals(this.mc.thePlayer.getUniqueID()) && var11) {
                ...
                if (!cheatbreaker.getCBNetHandler().getPlayersInVoiceChannel().removeIf(var1x -> var1x.equals(listener.getPlayerUUID()))) {
                    cheatbreaker.getCBNetHandler().getPlayersInVoiceChannel().add(listener.getPlayerUUID());
                }
            }
            ++var7;
        }
    ...
    public void keyTyped(char var1, int var2) throws IOException {
        ...
        if (var2 == 25 && blurColor.isOver()) {
            if ((Boolean) VoiceMenu.cheatbreaker.getGlobalSettings().guiBlur.getValue()) {
                this.mc.entityRenderer.stopUseShader();
            }
