...
public class MessagesElement extends DraggableElement {
    ...
    @SneakyThrows
    public static String handleCBProcessBytes(byte[] arrby) {
    ...
    public void handleElementDraw(float f, float f2, boolean bl) {
        ...
        ResourceLocation resourceLocation = CheatBreaker.getInstance().getHeadIcon(EnumChatFormatting.getTextWithoutFormattingCodes(this.friend.getName()), this.friend.getPlayerId());
        ...
        for (Friend friend : this.cb.getFriendsManager().getFriends().values()) {
            if (friend != this.friend && !this.cb.getFriendsManager().getReadMessages().containsKey(friend.getPlayerId()) && !friend.isOnline()) continue;
            ...
            ResourceLocation resourceLoc = CheatBreaker.getInstance().getHeadIcon(EnumChatFormatting.getTextWithoutFormattingCodes(friend.getName()), friend.getPlayerId());
    ...
    private void sendMessage() {
      ...
      //  CheatBreaker.getInstance().getAudioManager().playSound("short_whoosh1");
