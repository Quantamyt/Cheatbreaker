...
public class ProfilesListElement extends AbstractScrollableElement {
    ...
    public void loadProfiles() {
        new Thread(() -> {
            ...
            File file = new File(Minecraft.getMinecraft().mcDataDir + File.separator + "config" + File.separator + "cheatbreaker-client-" + Config.MC_VERSION.replaceAll("\\.", "-") + File.separator + "profiles");
            if (file.exists()) {
                for (File file2 : file.listFiles()) {
                    ...
                    CheatBreaker.getInstance().getConfigManager().moduleProfiles.add(new Profile(file2.getName().replace(".cfg", ""), false));
                }
            }
            for (Profile profile : CheatBreaker.getInstance().getConfigManager().moduleProfiles) {
