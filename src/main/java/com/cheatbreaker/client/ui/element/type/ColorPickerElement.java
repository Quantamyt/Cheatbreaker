...
//    private final Setting setting;
    ...
    private float pickerX2;
    private float pickerY2;
    private float pickerXDiff;
    private float pickerYDiff;
    ...
    public ColorPickerElement(Setting setting, float scaleFactor) {
        super(scaleFactor);
    ...
    public void handleDrawElement(int mouseX, int mouseY, float partialTicks) {
        this.height = this.expanded ? 130 : 18;
        this.pickerX = this.x + 56;
        this.pickerX2 = this.x + 176;
        this.pickerY = this.y + 25;
        this.pickerY2 = this.y + 119;
        this.pickerXDiff = this.pickerX2 - this.pickerX;
        this.pickerYDiff = this.pickerY2 - this.pickerY;
    ...
    private void drawHueSlider(int x, int y) {
        ...
        float f = (float)-1 + this.pickerYDiff * this.IIIlllIIIllIllIlIIIIIIlII;
        Gui.drawRect(this.pickerX + this.pickerXDiff + (float)4, this.pickerY + f, this.pickerX + this.pickerXDiff + (float)14, this.pickerY + f + (float)3, -822083584);
        Gui.drawRect(this.pickerX + this.pickerXDiff + (float)4, this.pickerY + f + 1.0f, this.pickerX + this.pickerXDiff + (float)14, this.pickerY + f + 2.0f, -805306369);
    }
    ...
    private void drawOpacityCheckerboard() {
        ...
        while ((float)n < this.pickerYDiff - (float)4) {
            if (!bl) {
                ...
                Gui.drawRect(this.pickerX + this.pickerXDiff + (float)23, this.pickerY + (float)n, this.pickerX + this.pickerXDiff + (float)27, this.pickerY + (float)n + (float)4, 0xFF909090);
                Gui.drawRect(this.pickerX + this.pickerXDiff + (float)19, this.pickerY + (float)n + (float)4, this.pickerX + this.pickerXDiff + (float)23, this.pickerY + (float)n + (float)8, -7303024);
            }
            bl = !bl;
            n += 4;
        }
    ...
    private void drawInstantColorSelectionBoxes(List<ColorPickerColorElement> list, int n, int n2, int n3, int n4, int n5) {
        int n6 = 0;
        int n7 = 0;
        int n8 = 8;
        for (ColorPickerColorElement colorPickerColorElementObj : list) {
            colorPickerColorElementObj.scale = this.scale;
            if (n6 == n8) {
                ++n7;
                n6 = 0;
            }
            if (list == this.colors) {
                ...
                if (colorPickerColorElementObj.isMouseInside(n3, n4)) {
                    ...
                    CheatBreaker.getInstance().ubuntuMedium16px.drawStringWithShadow("&" + string2, var12_12 + 14, var13_13, -1);
                }
            } else {
    ...
    public void handleMouseClick(int mouseX, int mouseY, int button) {
        ...
        if (this.setting == CheatBreaker.getInstance().getModuleManager().packTweaksMod.coloredStringColor) {
