...
public class ProfileElement extends AbstractModulesGuiElement {
    ...
    private final ResourceLocation checkmarkIcon = new ResourceLocation("client/icons/checkmark-64.png");
    ...
    public void handleDrawElement(int mouseX, int mouseY, float partialTicks) {
        ...
        boolean hoverCondition = mouseX > (this.x + 12) * 1.0F / CheatBreaker.getScaleFactor() && mouseX < (this.x + this.width - (this.profile.isNotEditable() ? 0 : 30)) * 1.0F / CheatBreaker.getScaleFactor() && this.isMouseInside(mouseX, mouseY);
        int n3 = 75;
    ...
    public void handleMouseClick(int mouseX, int mouseY, int button) {
        ...
        if (!this.profile.isNotEditable() && (bl3 || bl4)) {
            ...
            if (!this.profile.isNotEditable() && (file2 = (file = new File(Minecraft.getMinecraft().mcDataDir + File.separator + "config" + File.separator + "cheatbreaker-client-" + Config.MC_VERSION + "-" + CheatBreaker.getInstance().getGitBranch() + File.separator + "profiles")).exists() || file.mkdirs() ? new File(file + File.separator + this.profile.getName().toLowerCase() + ".cfg") : null).exists() && file2.delete()) {
                CheatBreaker.getInstance().profiles.removeIf(profile -> profile == this.profile);
