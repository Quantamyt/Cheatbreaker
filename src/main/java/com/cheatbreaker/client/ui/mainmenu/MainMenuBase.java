...
public class MainMenuBase extends AbstractGui {
    ...
    private final String summaryBuildInfo =
            CheatBreaker.getInstance().getGitBuildVersion().isEmpty()
                    || CheatBreaker.getInstance().getGitBuildVersion().toLowerCase().equals(CheatBreaker.getInstance().getGitBranch())
                    ? "CheatBreaker (" + CheatBreaker.getInstance().getGitCommitIdAbbrev() + "/" + CheatBreaker.getInstance().getGitBranch() + ")" : "CheatBreaker "
                    + CheatBreaker.getInstance().getGitBuildVersion() + " (" + CheatBreaker.getInstance().getGitCommitIdAbbrev() + "/"
    ...
    private final List<CBAccount> accountsList = new ArrayList<>();
    private float accountUsernameStringWidth = CheatBreaker.getInstance().robotoRegular13px.getStringWidth(Minecraft.getMinecraft().getSession().getUsername());
    String clientToken = "";
    ...
    private void loadAccounts() {
        ...
        this.accountUsernameStringWidth = (float) CheatBreaker.getInstance().robotoRegular13px.getStringWidth(Minecraft.getMinecraft().getSession().getUsername());
        for (Map<String, String> var11 : var2) {
            ...
            for (CBAccount account : this.accountsList) {
                if (account.getDisplayName().equals(var22.getDisplayName())) {
                    return;
                }
            }

            this.accountsList.add(var22);
            ...
            if (var13 > this.accountUsernameStringWidth) {
                this.accountUsernameStringWidth = var13;
            }
    ...
    public void initGui() {
        ...
        for (TextButtonElement button : this.horizontalButtonList) {
            button.setElementSize(124.0f + horizontalButtonXPosition, 6.0f, CheatBreaker.getInstance().robotoBold14px.getStringWidth(button.getString()) * 1.14F, 20.0f);
            horizontalButtonXPosition += CheatBreaker.getInstance().robotoBold14px.getStringWidth(button.getString()) + 12.0F;
        }
    ...
    public void getAccountListElementSize() {
        this.accountsButton.setElementSize(this.getScaledWidth() - 35.0f - this.accountsButton.setWidthFromAccountString(this.accountUsernameStringWidth), 7.0f, this.accountsButton.setWidthFromAccountString(this.accountUsernameStringWidth), 17.0f);
    }
    ...
    public void updateAccountElementAndLogin(String var1) {
        try {
            CBAccount var2 = null;
            ...
            if (var2 != null) {
                ...
                for (Session session : CheatBreaker.getInstance().playerSessions) {
                    if (session.getProfile().getId().toString().replaceAll("-", "").equalsIgnoreCase(var2.getUuid().replaceAll("-", ""))) {
                ...
                this.accountsButton.setResourceLocation(var2.getHeadIcon());
                this.getAccountListElementSize();
                CheatBreaker.getInstance().playerSessions.add(session);
                Minecraft.getMinecraft().setSession(session);
    ...
    public List<CBAccount> getAccountsList() {
