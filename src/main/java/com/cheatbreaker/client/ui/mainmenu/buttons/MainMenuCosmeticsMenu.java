...
public class MainMenuCosmeticsMenu extends GuiMainMenu {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (!CheatBreaker.getInstance().getWsNetHandler().isOpen()) {
            CheatBreaker.getInstance().playRegular16px.drawCenteredStringWithShadow("Unable to connect to the server.", this.width / 2F, this.height / 2F - 10, -1);
            CheatBreaker.getInstance().playRegular16px.drawCenteredStringWithShadow("Please try again later.", this.width / 2F, this.height / 2F + 4, -1);
        } else {
            RenderUtil.drawRoundedRect(this.width / 2F - 80, this.height / 2F - 78, this.width / 2F + 80, this.height / 2F + 100, 14, -1342177281);
            if (this.cosmetics.isEmpty()) {
                CheatBreaker.getInstance().playRegular16px.drawCenteredString("You don't own any cosmetics.", this.width / 2F, this.height / 2F + 4, -6381922);
            } else {
                CheatBreaker.getInstance().playBold18px.drawCenteredString("Cosmetics (" + this.cosmetics.size() + ")", this.width / 2F, this.height / 2F - 90, -1);
                ...
                for (Object cosmeticListElementObj : this.cosmetics) {
                    ...
                    if (++n3 - 1 < this.IIIIllIIllIIIIllIllIIIlIl * 5 || n3 - 1 >= (this.IIIIllIIllIIIIllIllIIIlIl + 1) * 5)
                        continue;
...
//        this.lIIIIIIIIIlIllIIllIlIIlIl(mouseX, mouseY);
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        for (Object cosmeticListElement : this.cosmetics) {
            if (++n4 - 1 < this.IIIIllIIllIIIIllIllIIIlIl * 5 || n4 - 1 >= (this.IIIIllIIllIIIIllIllIIIlIl + 1) * 5)
                continue;
