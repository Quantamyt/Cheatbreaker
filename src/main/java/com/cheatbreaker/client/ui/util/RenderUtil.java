...
public class RenderUtil {//TODO: Finish mapping this class
    ...
    public static void drawTexturedModalRect(float f, float f2, float f3, float f4, int n, int n2) {
        ...
        worldrenderer.pos(f, f2 + (float) n2, zero).tex(f3 * f5, (f4 + (float) n2) * f5).endVertex();
        worldrenderer.pos(f + (float) n, f2 + (float) n2, zero).tex((f3 + (float) n) * f5, (f4 + (float) n2) * f5).endVertex();
        worldrenderer.pos(f + (float) n, f2, zero).tex((f3 + (float) n) * f5, f4 * f5).endVertex();
        worldrenderer.pos(f, f2, zero).tex(f3 * f5, f4 * f5).endVertex();
        tessellator.draw();
    }
    ...
    public static RGBData getRgbValues(int color) {
        int red = (color >> 16 & 255) / 255;
        int green = (color >> 8 & 255) / 255;
        int blue = (color & 255) / 255;
