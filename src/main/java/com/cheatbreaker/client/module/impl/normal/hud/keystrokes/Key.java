 ...
 *
 * Draws the actual key for com.cheatbreaker.client.module.impl.normal.hud.keystrokes.ModuleKeyStrokes.
 */
public class Key {
    @Getter
    ...
    public void render(float x, float y, int textColor, int textColorPressed, int backgroundColor, int backgroundColorPressed, int borderColor, int borderColorPressed, boolean textShadow) {
        ...
        if (pressed && !this.pressed) {
            ...
            this.clicksPressedColor = new Color(CheatBreaker.getInstance().getModuleManager().keystrokesMod.clicksColor.getColorValue(), true);
            this.clicksUnpressedColor = new Color(CheatBreaker.getInstance().getModuleManager().keystrokesMod.clicksColorPressed.getColorValue(), true);
            ...
            this.clicksPressedColor = new Color(CheatBreaker.getInstance().getModuleManager().keystrokesMod.clicksColorPressed.getColorValue(), true);
            this.clicksUnpressedColor = new Color(CheatBreaker.getInstance().getModuleManager().keystrokesMod.clicksColor.getColorValue(), true);
        ...
        if ((float) (System.currentTimeMillis() - this.lastPressed) < textFadeTime) {
            ...
            int rp = textColorPressed >> 16 & 255;
            int gp = textColorPressed >> 8 & 255;
            int bp = textColorPressed & 255;
        ...
        if (this.keyCode == mc.gameSettings.keyBindJump.getKeyCode() && (Boolean) CheatBreaker.getInstance().getModuleManager().keystrokesMod.spacebarLine.getValue()) {
            ...
            float xPos = (x + this.width / 2.0F) - (float) (mc.fontRendererObj.getStringWidth(this.displayString) / 2) + 0.26F;
            float yPos = y + this.height / 2.0F - (float) (mc.fontRendererObj.FONT_HEIGHT / 2) + 0.65F;
            if ((Boolean) CheatBreaker.getInstance().getModuleManager().keystrokesMod.legacyPosition.getValue()) {
                xPos = (x + this.width / 2.0F) - (float) (mc.fontRendererObj.getStringWidth(this.displayString) / 2);
                yPos = y + this.height / 2.0F - (float) (mc.fontRendererObj.FONT_HEIGHT / 2) + 1.0f;
            }
            ...
            if (attackKeyCode || useItemKeyCode) {
                if (boxSize > 14.0F && clicksOption.equals("With Clicks")) {
                    ...
                    yPos = y + this.height / 2.0F - (float) (mc.fontRendererObj.FONT_HEIGHT / 2) + 6.0F;
                    ...
                    float xClicksPos = (x + this.width / 2.0F) - (mc.fontRendererObj.getStringWidth(clicks) / 2.0F) * cpsScale;
                    mc.fontRendererObj.drawString(clicks, xClicksPos / cpsScale, yPos / cpsScale, getCPSTextColor, textShadow);
                    GL11.glScalef(1.0F, 1.0F, 0.0F);
                    GL11.glPopMatrix();
                    yPos = y + this.height / 2.0F - (float) (mc.fontRendererObj.FONT_HEIGHT / 2) - 3.0F;
                    ...
                    xPos = x + this.width / 2.0F - (mc.fontRendererObj.getStringWidth(clicks) / 2.0F) * cpsScale + 0.26F;
                    yPos = y + this.height / 2.0F - (float) (mc.fontRendererObj.FONT_HEIGHT / 2) + (boxSize > 15.0F && !clicks.equals(this.displayString) ? 1.65F : 0.65F);
                    if ((Boolean) CheatBreaker.getInstance().getModuleManager().keystrokesMod.legacyPosition.getValue()) {
                        xPos = x + this.width / 2.0F - (mc.fontRendererObj.getStringWidth(clicks) / 2.0F) * cpsScale;
                        yPos = y + this.height / 2.0F - (float) (mc.fontRendererObj.FONT_HEIGHT / 2) + (boxSize > 15.0F && !clicks.equals(this.displayString) ? 2.0F : 1.0F);
                    }
                    mc.fontRendererObj.drawString(clicks, xPos / cpsScale, yPos / cpsScale, getTextColor, textShadow);
                    GL11.glScalef(1.0F, 1.0F, 0.0F);
                    GL11.glPopMatrix();
                }
            }
            ...
            if (!clicksOption.equals("Replace Clicks")) {
                mc.fontRendererObj.drawString(this.displayString, xPos, yPos, getTextColor, textShadow);
            }
