...
/**
 * @Module - ModuleTeamMates
 * @see AbstractModule
 *
 * This module is the first of its kind. This module is more widely known as "Team View".
 * This will only show arrows above your teammates with servers using the most up-to-date LunarClientAPI Bukkit plugin.
 */
@Getter
public class ModuleTeamMates {
    ...
    private void onDraw(GuiDrawEvent event) {
        if (this.teammates.isEmpty()) return;
        ...
        for (CBTeammate cBTeammate : this.teammates) {
            ...
            if (entityPlayer == null) {
                ...
                this.renderArrow(event.getScaledResolution(), cBTeammate, (float) d4, (float) d5, (float) d6, intBuffer, vec3, (int) d7);
                continue;
            }
            ...
            this.renderArrow(event.getScaledResolution(), cBTeammate, f5, f6, f7, intBuffer, vec3, (int) d8);
        }
    }

    private void renderArrow(ScaledResolution scaledResolution, CBTeammate teammate, float f, float f2, float f3, IntBuffer intBuffer, Vec3 vec3, int n) {
        ...
        if (arrowLocation != null) {
            if ((Boolean) CheatBreaker.getInstance().getGlobalSettings().showOffScreenMarker.getValue()) {
                this.renderArrow(teammate, arrowLocation, 0.0f, 0.0f);
            }
        } else {
            this.renderArrow(teammate, n2, (float) n4, (float) n3);
            if (n > 40 && (Boolean) CheatBreaker.getInstance().getGlobalSettings().showDistance.getValue()) {
                this.mc.fontRendererObj.drawCenteredStringWithShadow("(" + n + "m)", 0, 10, -1);
            }
        }
        GL11.glPopMatrix();
    }

    private void renderArrow(CBTeammate teammate, TeammateArrowLocation arrowLocation, float f, float f2) {
        ...
        switch (arrowLocation) {
            case TOP:
                ...
                wr.pos(f3 / 2.0f, f4 / 2.0f, 0.0).endVertex();
                wr.pos(-f3 / 2.0f, 0.0, 0.0).endVertex();
                wr.pos(f3 / 2.0f, -f4 / 2.0f, 0.0).endVertex();
                wr.pos(-f3 / 2.0f, 0.0, 0.0).endVertex();
                tes.draw();
                break;
            ...
            case BOTTOM:
                wr.begin(7, DefaultVertexFormats.POSITION);
                wr.pos(-f3 / 2.0f, -f4 / 2.0f, 0.0).endVertex();
                wr.pos(0.0, f4 / 2.0f, 0.0).endVertex();
                wr.pos(f3 / 2.0f, -f4 / 2.0f, 0.0).endVertex();
                wr.pos(0.0, f4 / 2.0f, 0.0).endVertex();
                tes.draw();
                break;
    ...
    private void renderArrow(CBTeammate teammate, float f, float f2, float f3) {
        ...
        wr.begin(7, DefaultVertexFormats.POSITION);
        wr.pos(-f, f2, 0.0).endVertex();
        wr.pos(-f, f2 + f3 / 2.0f, 0.0).endVertex();
        wr.pos(f, f2 + f3 / 2.0f, 0.0).endVertex();
        wr.pos(f, f2, 0.0).endVertex();
        tessellator.draw();
        GL11.glRotatef(90.0f, 0.0f, 0.0f, -1.0f);
        ...
        wr.begin(7, DefaultVertexFormats.POSITION);
        wr.pos(-f / 2.0f + 1.0f, f2, 0.0).endVertex();
        wr.pos(-f / 2.0f + 1.0f, f2 + f3 / 2.0f, 0.0).endVertex();
        wr.pos(f, f2 + f3 / 2.0f, 0.0).endVertex();
        wr.pos(f, f2, 0.0).endVertex();
        tessellator.draw();
        GL11.glPopMatrix();
