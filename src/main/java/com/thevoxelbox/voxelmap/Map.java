...
public class Map implements Runnable, IMap {
    ...
    private final MapData[] mapData = new MapData[4];
    private final MapChunkCache[] chunkCache = new MapChunkCache[4];
    private final GLBufferedImage[] map = new GLBufferedImage[4];
    ...
    private final int[] lastLightmapValues = {-16777216,
            -16777216,
            -16777216,
            -16777216,
            -16777216,
    ...
    private final String[] sMenu = new String[8];
    ...
    private final boolean worldDownloaderExists = false;
    ...
    public void onTickInGame(Minecraft mc) {
        ...
        if (this.threading) {
            ...
            if (!this.options.hide) {
                mapCalc(this.doFullRender);
                if (!this.doFullRender || ModuleMiniMap.rule != MiniMapRules.NEUTRAL && ModuleMiniMap.rule != MiniMapRules.ALLOWED) {
                    boolean realTimeUpdate = (!this.options.dlSafe) && ((!this.worldDownloaderExists));
                    this.chunkCache[this.zoom].centerChunks(this.lastX, this.lastZ);
                    this.chunkCache[this.zoom].calculateChunks(realTimeUpdate);
                }
        ...
        if (this.enabled && (ModuleMiniMap.rule == MiniMapRules.NEUTRAL || ModuleMiniMap.rule == MiniMapRules.ALLOWED)) {
            drawMinimap(mc);
        }
    ...
    public void drawMinimap(Minecraft mc) {
        ...
        if (!(this.options.hide || ModuleMiniMap.rule != MiniMapRules.NEUTRAL && ModuleMiniMap.rule != MiniMapRules.ALLOWED)) {
            GL11.glEnable(2929);
                ...
                if (this.game.thePlayer != null) {
                    try {
                        Method tfCatch = ReflectionUtils
                                .getMethodByType(0, EntityPlayer.class, String.class);
    ...
    private void mapCalc(boolean full) {
        ...
        if (this.game.thePlayer.dimension != -1) {
            caves = (this.options.cavesAllowed.booleanValue()) && (this.options.showCaves) &&
