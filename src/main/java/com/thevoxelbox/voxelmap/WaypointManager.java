...
public class WaypointManager implements IWaypointManager {
	...
	public void newWorld(int dimension) {
		synchronized (this.lock) {
			for (Waypoint pt : this.wayPts) {
				pt.inDimension = (pt.dimensions.size() == 0) || (pt.dimensions.contains(Integer.valueOf(dimension)));
			}
	...
	public void newSubWorldDescriptor(String descriptor) {
		...
		synchronized (this.lock) {
			...
			for (Waypoint pt : this.wayPts) {
				pt.inWorld = (currentSubWorldDescriptorScrubbed == "") || (pt.world == "") ||
				...
				if (!pt.name.startsWith("^")) {
					...
					out.println(
							"name:" + scrubName(pt.name) + ",x:" + pt.x + ",z:" + pt.z + ",y:" + pt.y + ",enabled:" +
									pt.enabled + ",red:" + pt.red + ",green:" + pt.green + ",blue:" +
	...
	public ArrayList<Waypoint> getWaypts() {
		return this.wayPts;
	}

	public ArrayList<Waypoint> getOld2dWayPts() {
		return this.old2dWayPts;
	}

	public EntityWaypointContainer getEntityWaypointContainer() {
		return this.entityWaypointContainer;
	}
}
