    ...
    private static IIcon getConnectedTextureRandom(ConnectedProperties cp, IBlockAccess blockAccess, Block block, int x, int y, int z, int side) {
        if (cp.tileIcons.length == 1) {
            ...
            if (cp.linked) {
                ...
                for (Block index = blockAccess.getBlock(x, rand, z); index == block; index = blockAccess.getBlock(x, rand, z)) {
                    y = rand--;
            ...
            rand = Config.getRandom(x, y, z, face) & Integer.MAX_VALUE;
            int var13 = 0;
