...
public class CrashReport
{
    ...
    {
        // CheatBreaker Crash Report System
        try {
            StackTraceElement[] stackTrace;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append(causeIn.getClass().getSimpleName()).append(": ").append(causeIn.getLocalizedMessage());
            for (StackTraceElement element : stackTrace = causeIn.getStackTrace()) {
                stringBuilder.append("\n").append("\t").append("at ").append(element);
            }
            UUID uuid = UUID.randomUUID();
            StringSelection reportID = new StringSelection("CB-" + uuid);
            Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
            clipboard.setContents(reportID, null);
//            CrashHandler.handleCrash(causeIn.getStackTrace());
            sendCrashReport(uuid.toString(), stringBuilder.toString());
            if (causeIn.getClass().getSimpleName().equals("OutOfMemoryError")) {
        ...
        // Normal Minecraft Crash Report System
