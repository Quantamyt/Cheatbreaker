...
public abstract class LayerArmorBase<T extends ModelBase> implements LayerRenderer<EntityLivingBase>
{
    ...
    {
        return CheatBreaker.getInstance().getModuleManager().hitColorMod.affectArmor.getBooleanValue() && CheatBreaker.getInstance().getModuleManager().hitColorMod.isEnabled();
    }
    ...
    private void renderGlint(EntityLivingBase entitylivingbaseIn, T modelbaseIn, float p_177183_3_, float p_177183_4_, float partialTicks, float p_177183_6_, float p_177183_7_, float p_177183_8_, float scale)
    {
        ...
        if ((!Config.isShaders() || !Shaders.isShadowPass) && gm.isEnabled() && gm.armorGlint.getBooleanValue())
        {
            int glintColor = gm.armorGlintColor.getColorValue();
            float glintA = (float)(glintColor >> 24 & 255) / 255.0F;
            float glintR = (float)(glintColor >> 16 & 255) / 255.0F;
            float glintG = (float)(glintColor >> 8 & 255) / 255.0F;
            float glintB = (float)(glintColor & 255) / 255.0F;
            ...
            GlStateManager.color(glintR, glintG, glintB, glintA);
