...
public class RenderPlayer extends RendererLivingEntity<AbstractClientPlayer>
{
    ...
    protected void renderOffsetLivingLabel(AbstractClientPlayer entityIn, double x, double y, double z, String str, float p_177069_9_, double p_177069_10_)
    {
        ...
        if (CheatBreaker.getInstance().getCBNetHandler().getCustomNameTags().containsKey(entityIn.getUniqueID())) {
            for (String string2 : CheatBreaker.getInstance().getCBNetHandler().getCustomNameTags().get(entityIn.getUniqueID())) {
                if (!string2.contains(entityIn.getNameClear())) {
                    y += ((float)this.getFontRendererFromRenderManager().FONT_HEIGHT * 1.15F * p_177069_9_);
                    this.renderLivingLabel(entityIn, string2, x, y, z, 64);
                }
            }
        }
    }
