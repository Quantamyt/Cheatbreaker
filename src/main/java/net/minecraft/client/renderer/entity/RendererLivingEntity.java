...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    protected boolean setBrightness(T entitylivingbaseIn, float partialTicks, boolean combineTextures) {
        ...
        if (!flag && !flag1) {
            ...
            float brightness = ModuleOneSevenVisuals.redArmor.getBooleanValue() ? 0.73f : 1.0F;
            if (flag1) {
                ModuleHitColor hc = CheatBreaker.getInstance().getModuleManager().hitColorMod;
                if (hc.isEnabled()) {
                    f = hc.affectedBrightness.getBooleanValue() ? f : 1.0F;
                    float hitA = (float)(hc.hitColor.getColorValue() >> 24 & 255) / 255.0F;
                    if (!hc.fade.getValue().equals("None")) {
                        float percent = 1.0F - (float)entitylivingbaseIn.hurtTime / 10.0F;
                        if (hc.fade.getValue().equals("Linear In/Out")) {
                            percent = percent < 0.5F ? percent / 0.5F : (1.0F - percent) / 0.5F;
                        } else if (hc.fade.getValue().equals("Linear Out")) {
                    ...
                    this.brightnessBuffer.put((float) (hc.hitColor.getColorValue() >> 16 & 255) / 255.0F * brightness * f);
                    this.brightnessBuffer.put((float) (hc.hitColor.getColorValue() >> 8 & 255) / 255.0F * brightness * f);
                    this.brightnessBuffer.put((float) (hc.hitColor.getColorValue() & 255) / 255.0F * brightness * f);
    ...
    public void renderName(T entity, double x, double y, double z) {
        ...
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, entity, this, x, y, z)) {
            if (this.canRenderName(entity) || (entity.equals(Minecraft.getMinecraft().thePlayer) && nametagMod.isEnabled() && nametagMod.showOwnNametag.getBooleanValue())) {

                double d0 = entity.getDistanceSqToEntity(this.renderManager.livingPlayer);
                float f = entity.isSneaking() ? NAME_TAG_RANGE_SNEAK : NAME_TAG_RANGE;


                if (d0 < (double) (f * f) && !CheatBreaker.getInstance().hideNameTags) {
                    ...
                    if (nickHider.isEnabled() && nickHider.hideRealName.getBooleanValue()) {
                        if (!nickHider.customNameString.getStringValue().equals(Minecraft.getMinecraft().getSession().getUsername())) {
                            entityName = entityName.replaceAll(Minecraft.getMinecraft().getSession().getUsername(), nickHider.customNameString.getStringValue());
                        } else {
                            entityName = entityName.replaceAll(Minecraft.getMinecraft().getSession().getUsername(), "You");
                        }
                    }
                    ...
                    if (entity.isSneaking()) {
                        ...
                        GlStateManager.enableTexture2D();
                        GlStateManager.depthMask(true);
                        fontrenderer.drawString(entityName, -fontrenderer.getStringWidth(entityName) / 2, 0, 553648127, nametagMod.isEnabled() && nametagMod.textShadow.getBooleanValue());
                        GlStateManager.enableLighting();
                        GlStateManager.disableBlend();
                        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
                        GlStateManager.popMatrix();
                    } else {
                        this.renderOffsetLivingLabel(entity, x, y - (entity.isChild() ? (double) (entity.height / 2.0F) : 0.0D), z, entityName, 0.02666667F, d0);
                    }
                }
            }
