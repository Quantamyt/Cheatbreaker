...
public class RenderItem implements IResourceManagerReloadListener
{
    ...
    {
        ModuleEnchantmentGlint gm = CheatBreaker.getInstance().getModuleManager().enchantmentGlintMod;
        ...
        if (!Config.isCustomItems() || CustomItems.isUseGlint())
        {
            if (!Config.isShaders() || !Shaders.isShadowPass)
            {
                ...
                if (gm.isEnabled()) this.renderModel(model, gm.glintColor.getColorValue());
    ...
    protected void renderItemModelTransform(ItemStack stack, IBakedModel model, TransformType cameraTransformType)
    {
        ...
        EntityPlayer p;
        ItemStack heldStack;
        if (cameraTransformType == ItemCameraTransforms.TransformType.THIRD_PERSON && this.lastEntityToRenderFor instanceof EntityPlayer && (heldStack = (p = (EntityPlayer)((Object)this.lastEntityToRenderFor)).getHeldItem()) != null && p.getItemInUseCount() > 0 && heldStack.getItemUseAction() == EnumAction.BLOCK) {
            AnimationHandler.INSTANCE.doSwordBlock3rdPersonTransform();
        }
    ...
    public void renderItemIntoGUI(ItemStack stack, int x, int y)
    {
        ...
        if (Reflector.ForgeHooksClient_handleCameraTransforms.exists())
        {
            ibakedmodel = (IBakedModel)Reflector.call(Reflector.ForgeHooksClient_handleCameraTransforms, new Object[] {ibakedmodel, TransformType.GUI});
        }
        else
        {
            ibakedmodel.getItemCameraTransforms().applyTransform(TransformType.GUI);
        }
