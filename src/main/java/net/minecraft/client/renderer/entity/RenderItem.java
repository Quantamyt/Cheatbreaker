...
public class RenderItem extends Render {
    ...
    private void renderDroppedItem(EntityItem p_77020_1_, IIcon p_77020_2_, int p_77020_3_, float p_77020_4_, float p_77020_5_, float p_77020_6_, float p_77020_7_) {
        ...
        if (this.renderManager.options.fancyGraphics) {
            ...
            if (renderInFrame) {
                ...
                if ((Boolean) CheatBreaker.getInstance().getModuleManager().packTweaksMod.faceCamera.getValue() && CheatBreaker.getInstance().getModuleManager().packTweaksMod.isEnabled()) {
                    GL11.glRotatef(360.0f - this.renderManager.playerViewY, 0.0f, 1.0f, 0.0f);
                    if ((Boolean) CheatBreaker.getInstance().getModuleManager().packTweaksMod.yRotation.getValue()) {
            ...
            for (int var20 = 0; var20 < var24; ++var20) {
                ...
                if (var18.hasEffect() && glintModule.isEnabled() && (Boolean) glintModule.droppedItemGlint.getValue()) {
                    GL11.glDepthFunc(GL11.GL_EQUAL);
                    GL11.glDisable(GL11.GL_LIGHTING);
                    this.renderManager.renderEngine.bindTexture(RES_ITEM_GLINT);
                    GL11.glEnable(GL11.GL_BLEND);
                    GL11.glBlendFunc(GL11.GL_SRC_COLOR, GL11.GL_ONE);
                    int getGlintColor = glintModule.droppedItemGlintColor.getColorValue();
                    float glintA = (float)(getGlintColor >> 24 & 0xFF) / 255.0f;
                    float glintR = (float)(getGlintColor >> 16 & 0xFF) / 255.0f;
                    float glintG = (float)(getGlintColor >> 8 & 0xFF) / 255.0f;
                    float glintB = (float)(getGlintColor & 0xFF) / 255.0f;
                    float a = glintA * 0.76f;
                    GL11.glColor4f(glintR * a, glintG * a, glintB * a, 1.0F);
            ...
            for (int var27 = 0; var27 < p_77020_3_; ++var27) {
                ...
                if (!renderInFrame) {
                    ...
                    if ((Boolean) CheatBreaker.getInstance().getModuleManager().packTweaksMod.yRotation.getValue() && CheatBreaker.getInstance().getModuleManager().packTweaksMod.isEnabled()) {
                        GL11.glRotatef(-this.renderManager.playerViewX, 1.0F, 0.0F, 0.0F);
                    }
                }
    ...
    public void renderItemAndEffectIntoGUI(FontRenderer p_82406_1_, TextureManager p_82406_2_, final ItemStack p_82406_3_, int p_82406_4_, int p_82406_5_) {
        if (p_82406_3_ != null) {
            ...
            ModuleEnchantmentGlint glintModule = CheatBreaker.getInstance().getModuleManager().enchantmentGlintMod;
            if (p_82406_3_.hasEffect() && glintModule.isEnabled() && (Boolean) glintModule.inventoryGlint.getValue()) {
                GL11.glDepthFunc(GL11.GL_EQUAL);
                GL11.glDisable(GL11.GL_LIGHTING);
                ...
                GL11.glEnable(GL11.GL_ALPHA_TEST);
                GL11.glEnable(GL11.GL_BLEND);
                int var8 = p_82406_3_.getItem().getColorFromItemStack(p_82406_3_, 0);
                float var10 = (float)(var8 >> 16 & 0xFF) / 255.0f;
                float var11 = (float)(var8 >> 8 & 0xFF) / 255.0f;
                float var9 = (float)(var8 & 0xFF) / 255.0f;
                int getGlintColor = glintModule.inventoryGlintColor.getColorValue();
                float glintA = (float)(getGlintColor >> 24 & 0xFF) / 255.0f;
                float glintR = (float)(getGlintColor >> 16 & 0xFF) / 255.0f;
                float glintG = (float)(getGlintColor >> 8 & 0xFF) / 255.0f;
                float glintB = (float)(getGlintColor & 0xFF) / 255.0f;
                if (var8 != 0xFFFFFF && (Boolean)glintModule.showPotionColor.getValue()) {
                    GL11.glColor4f(var10 * glintA, var11 * glintA, var9 * glintA, 1.0F);
                } else {
                    GL11.glColor4f(glintR * glintA, glintG * glintA, glintB * glintA, 1.0F);
                }
                if (var8 != 0xFFFFFF && !(Boolean)glintModule.excludePotionGlint.getValue() || var8 == 0xFFFFFF && !(Boolean)glintModule.onlyRenderPotionGlint.getValue()) {
                    if (glintModule.boxBoundary.getValue().equals("Entire Slot") ) {
                        this.renderGlint(p_82406_4_ * 431278612 + p_82406_5_ * 32178161, p_82406_4_ - 2, p_82406_5_ - 2, 20, 20);
                    } else {
                        this.renderGlint(p_82406_4_ * 431278612 + p_82406_5_ * 32178161, p_82406_4_, p_82406_5_, 16, 16);
                    }
                }
                OpenGlHelper.glBlendFunc(770, 771, 1, 0);
    ...
    private void renderGlint(int p_77018_1_, int p_77018_2_, int p_77018_3_, int p_77018_4_, int p_77018_5_) {
        for (int var6 = 0; var6 < 2; ++var6) {
            OpenGlHelper.glBlendFunc((Boolean) CheatBreaker.getInstance().getModuleManager().enchantmentGlintMod.shinyPots.getValue() ? 773 : 772, 1, 0, 0);
