...
import com.cheatbreaker.client.module.impl.normal.vanilla.ModuleHitboxes;
import com.cheatbreaker.client.ui.util.RenderUtil;
import com.cheatbreaker.client.util.render.RGBData;
...
public class RenderManager {
    ...
    @Getter
    private double renderPosY; // Added getters for Hologram.java
    @Getter
    ...
    public void cacheActiveRenderInfo(World worldIn, FontRenderer textRendererIn, Entity livingPlayerIn, Entity pointedEntityIn, GameSettings optionsIn, float partialTicks) {
        ...
        if (livingPlayerIn instanceof EntityLivingBase && ((EntityLivingBase) livingPlayerIn).isPlayerSleeping()) {
            ...
            ModuleDragToLook var11 = CheatBreaker.getInstance().getModuleManager().dragToLook;
            if (var11.enabled) {
                this.playerViewY = var11.prevRotationYaw + (var11.rotationYaw - var11.prevRotationYaw) * partialTicks;
                this.playerViewX = var11.prevRotationPitch + (var11.rotationPitch - var11.prevRotationPitch) * partialTicks;
            } else {
        ...
        if (optionsIn.thirdPersonView == 2) {
            ...
            this.playerViewX = -this.playerViewX; // Fix - Tellinq
        }
    ...
    public boolean doRenderEntity(Entity entity, double x, double y, double z, float entityYaw, float partialTicks, boolean hideDebugBox) {
        ...
        try {
            ...
            if (render != null && this.renderEngine != null) {
                ...
                if (CheatBreaker.getInstance().getModuleManager().hitboxesMod.isEnabled() || (this.debugBoundingBox && !entity.isInvisible() && !hideDebugBox)) {
                    try {
    ...
    private void renderDebugBoundingBox(Entity entityIn, double x, double y, double z, float entityYaw, float partialTicks) {
        ModuleHitboxes moduleHitboxes = CheatBreaker.getInstance().getModuleManager().hitboxesMod;
        boolean showOwn = moduleHitboxes.showOwnHitbox.getBooleanValue();
        boolean isThePlayer = entityIn == Minecraft.getMinecraft().thePlayer;
        int color = moduleHitboxes.isEnabled() ? isThePlayer ? moduleHitboxes.ownHitboxColor.getColorValue() : moduleHitboxes.color.getColorValue() : 16777215;
        ...
        if (!Shaders.isShadowPass) {
            ...
            if (entityIn instanceof EntityLivingBase) {
                float f1 = 0.01F;
                AxisAlignedBB boundingBox = new AxisAlignedBB(x - (double) f, y + (double) entityIn.getEyeHeight() - 0.009999999776482582D, z - (double) f, x + (double) f, y + (double) entityIn.getEyeHeight() + 0.009999999776482582D, z + (double) f);

                RGBData rgbData = RenderUtil.getRgbValues(color);
                RenderGlobal.drawOutlinedBoundingBox(boundingBox, rgbData.getRed(), rgbData.getGreen(), rgbData.getBlue(), rgbData.getAlpha());
            }
