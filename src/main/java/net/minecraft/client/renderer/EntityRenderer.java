...
import com.cheatbreaker.client.module.impl.normal.animation.util.SneakHandler;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    private void hurtCameraEffect(float partialTicks) {
        ...
        if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
            ...
            GlStateManager.rotate(-f * (perspectiveMod.isEnabled() ? hurtCameraIntensity : 14.0F), 0.0F, 0.0F, 1.0F);
            GlStateManager.rotate(f2, 0.0F, 1.0F, 0.0F);
        }
    }
    ...
    private void freelookOrientCamera(float partialTicks) {
        ModuleDragToLook var2 = CheatBreaker.getInstance().getModuleManager().dragToLook;
        Entity entity = this.mc.getRenderViewEntity();
        float f = this.mc.getRenderViewEntity() != this.mc.thePlayer ? entity.getEyeHeight() : SneakHandler.getInstance().getEyeHeight(partialTicks);
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                GlStateManager.rotate(var2.rotationPitch - f2, 1.0F, 0.0F, 0.0F);
                GlStateManager.rotate(var2.rotationYaw - f1, 0.0F, 1.0F, 0.0F);
                GlStateManager.translate(0.0F, 0.0F, (float) (-d3));
                GlStateManager.rotate(f1 - var2.rotationYaw, 0.0F, 1.0F, 0.0F);
                GlStateManager.rotate(f2 - var2.rotationPitch, 1.0F, 0.0F, 0.0F);
            }
        } else {
        ...
        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
            if (!this.mc.gameSettings.debugCamEnable) {
                float f6 = var2.prevRotationYaw + (var2.rotationYaw - var2.prevRotationYaw) * partialTicks + 180.0F;
                float f7 = var2.prevRotationPitch + (var2.rotationPitch - var2.prevRotationPitch) * partialTicks;
                float f8 = 0.0F;
    ...
    public void setupCameraTransform(float partialTicks, int pass) {
        ...
        if (this.mc.gameSettings.viewBobbing) {
            if (perspectiveMod.isEnabled()) {
                perspectiveMod.setupViewBobbing(partialTicks, perspectiveMod.bobScreen);
            } else {
        ...
        if (CheatBreaker.getInstance().getModuleManager().dragToLook.enabled) {
            this.freelookOrientCamera(partialTicks);
        } else {
    ...
    public void renderHand(float p_renderHand_1_, int p_renderHand_2_, boolean p_renderHand_3_, boolean p_renderHand_4_, boolean p_renderHand_5_) {
        if (!this.debugView) {
            ...
            if (p_renderHand_3_) {
                ...
                if (this.mc.gameSettings.viewBobbing) {
                    if (perspectiveMod.isEnabled()) {
                        perspectiveMod.setupViewBobbing(p_renderHand_1_, perspectiveMod.bobScreen);
                    } else {
        ...
        if (this.mc.inGameHasFocus && flag) {
            ...
            if (!var9.enabled) {
                ...
                if (this.mc.gameSettings.smoothCamera) {
                    ...
                    this.smoothCamPartialTicks = partialTicks;
                    f2 = this.smoothCamFilterX * var18;
                    f3 = this.smoothCamFilterY * var18;
                    var9.setAngles(f2, f3 * (float) i);
                } else {
                    this.smoothCamYaw = 0.0F;
                    this.smoothCamPitch = 0.0F;
                    var9.setAngles(f2, f3 * (float) i);
        ...
        if (!this.debugView) {
            ...
            if (this.mc.objectMouseOver != null && entity.isInsideOfMaterial(Material.water) && flag1) {
                ...
                if (CheatBreaker.getInstance().getModuleManager().blockOverlayMod.isEnabled()) {
                    CheatBreaker.getInstance().getModuleManager().blockOverlayMod.drawSelectionBox(entityplayer, this.mc.objectMouseOver, 0, partialTicks, this.mc.theWorld);
                } else {
...
//                renderglobal.drawSelectionBox(entityplayer, this.mc.objectMouseOver, 0, partialTicks);
        ...
        if (flag1 && this.mc.objectMouseOver != null && !entity.isInsideOfMaterial(Material.water)) {
            ...
            if ((!Reflector.ForgeHooksClient_onDrawBlockHighlight.exists() || !Reflector.callBoolean(Reflector.ForgeHooksClient_onDrawBlockHighlight, new Object[]{renderglobal, entityplayer1, this.mc.objectMouseOver, Integer.valueOf(0), entityplayer1.getHeldItem(), Float.valueOf(partialTicks)})) && !this.mc.gameSettings.hideGUI) {
                if (CheatBreaker.getInstance().getModuleManager().blockOverlayMod.isEnabled()) {
                    CheatBreaker.getInstance().getModuleManager().blockOverlayMod.drawSelectionBox(entityplayer1, this.mc.objectMouseOver, 0, partialTicks, this.mc.theWorld);
                } else {
    ...
    protected void renderRainSnow(float partialTicks) {
        ...
        if (f5 > 0.0F || envChanger.isEnabled()) {
            ...
            for (int k1 = k - i1; k1 <= k + i1; ++k1) {
                for (int l1 = i - i1; l1 <= i + i1; ++l1) {
                    ...
                    mutableblockpos.set(l1, 0, k1);
                    BiomeGenBase biomegenbase = world.getBiomeGenForCoords(mutableblockpos);

                    if (biomegenbase.canRain() || biomegenbase.getEnableSnow()) {
                        int j2 = world.getPrecipitationHeight(mutableblockpos).getY();
                        int k2 = j - i1;
                        int l2 = j + i1;

                        if (k2 < j2) {
                            // passing through blocks logic
                            k2 = (envChanger.isEnabled() && allowThroughBlocks ? 0 :  j2);
                        }
                        ...
                        if (k2 != l2 || (envChanger.isEnabled() && envChanger.customWeather.getBooleanValue())) {
                            this.random.setSeed((l1 * l1 * 3121 + l1 * 45238971 ^ k1 * k1 * 418711 + k1 * 13761));
                            mutableblockpos.set(l1, k2, k1);
                            float f1 = biomegenbase.getFloatTemperature(mutableblockpos);
                            ...
                            else shouldRain = world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15F;
                            ...
                            } else if (world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) < 0.15F || (envChanger.weatherType.getStringValue().equals("Snow") && envChanger.customWeather.getBooleanValue() && envChanger.isEnabled())) {
                                if (j1 != 1) {
                                    if (j1 >= 0) {
                                        tessellator.draw();
                                    }
    ...
    private void frameInit() {
        ...
        if (world != null) {
            if (Config.getNewRelease() != null) {
                ...
                chatcomponenttext.setChatStyle((new ChatStyle()).setChatClickEvent(new ClickEvent(Action.OPEN_URL, "https://optifine.net/downloads")));
