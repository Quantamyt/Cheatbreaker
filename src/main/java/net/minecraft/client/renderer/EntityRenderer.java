...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    /**
     * CheatBreaker - Counts up when Slow Chunk Loading is active and stops chunk loading from occurring when the timer is not 0.
     */
    ...
    private static final ResourceLocation guiBlurResourceLocation = new ResourceLocation("shaders/post/animblur.json");
    private static final ResourceLocation oldMotionBlurResourceLocation = new ResourceLocation("shaders/post/motionblur.json");
    private static final ResourceLocation lunarMotionBlurResourceLocation = new ResourceLocation("shaders/post/lunar_motionblur.json");
    ...
    public void loadGuiBlurShader() {
        if (OpenGlHelper.isFramebufferEnabled() && OpenGlHelper.shadersSupported) {
            if (this.theShaderGroup != null) {
                this.theShaderGroup.deleteShaderGroup();
            }
            try {
                this.theShaderGroup = new ShaderGroup(this.mc.getTextureManager(), this.resourceManager, this.mc.getFramebuffer(), guiBlurResourceLocation);
                this.theShaderGroup.createBindFramebuffers(this.mc.displayWidth, this.mc.displayHeight);
    ...
    public void loadMotionBlurShader() {
        if (OpenGlHelper.isFramebufferEnabled() && OpenGlHelper.shadersSupported) {
            if (this.theShaderGroup != null) {
                this.theShaderGroup.deleteShaderGroup();
            }
            ResourceLocation motionBlurLocation = (Boolean) CheatBreaker.getInstance().getModuleManager().motionBlurMod.oldBlur.getValue() ? oldMotionBlurResourceLocation : lunarMotionBlurResourceLocation;
            try {
                this.theShaderGroup = new ShaderGroup(this.mc.getTextureManager(), this.resourceManager, this.mc.getFramebuffer(), motionBlurLocation);
                this.theShaderGroup.createBindFramebuffers(this.mc.displayWidth, this.mc.displayHeight);
            } catch (Exception exception) {
                logger.warn("Failed to load shader: " + motionBlurLocation, exception);
            }
        }
    }
    ...
    private void hurtCameraEffect(float par1) {
        ...
        boolean disableHurtCam = perspectiveMod.isEnabled() && !perspectiveMod.damageAffectsCamera.getBooleanValue();
        float hurtCameraIntensity = perspectiveMod.hurtCameraIntensity.getFloatValue();
        ...
        if (var3 >= 0.0F) {
            ...
            GL11.glRotatef(-var3 * (perspectiveMod.isEnabled() ? hurtCameraIntensity : 14.0F), 0.0F, 0.0F, 1.0F);
            GL11.glRotatef(var4, 0.0F, 1.0F, 0.0F);
        }
    }
    ...
    private void freelookOrientCamera(float var1) {
        ModuleDragToLook var2 = CheatBreaker.getInstance().getModuleManager().dragToLook;
        EntityLivingBase var3 = this.mc.renderViewEntity;
        float var4 = var3.yOffset - 1.62f;
        double var5 = var3.prevPosX + (var3.posX - var3.prevPosX) * (double)var1;
        double var7 = var3.prevPosY + (var3.posY - var3.prevPosY) * (double)var1 - (double)var4;
        double var9 = var3.prevPosZ + (var3.posZ - var3.prevPosZ) * (double)var1;
        GL11.glRotatef(this.prevCamRoll + (this.camRoll - this.prevCamRoll) * var1, 0.0f, 0.0f, 1.0f);
        if (var3.isPlayerSleeping()) {
            var4 = (float)((double)var4 + 1.0);
            GL11.glTranslatef(0.0f, 0.3f, 0.0f);
            if (!this.mc.gameSettings.debugCamEnable) {
                Block var11 = this.mc.theWorld.getBlock(MathHelper.floor_double(var3.posX), MathHelper.floor_double(var3.posY), MathHelper.floor_double(var3.posZ));
                if (var11 == Blocks.bed) {
                    int var12 = this.mc.theWorld.getBlockMetadata(MathHelper.floor_double(var3.posX), MathHelper.floor_double(var3.posY), MathHelper.floor_double(var3.posZ));
                    int var13 = var12 & 3;
            ...
            double var28 = this.thirdPersonDistanceTemp + (this.thirdPersonDistance - this.thirdPersonDistanceTemp) * var1;
            if (this.mc.gameSettings.debugCamEnable) {
                float var29 = this.prevDebugCamYaw + (this.debugCamYaw - this.prevDebugCamYaw) * var1;
                float var14 = this.prevDebugCamPitch + (this.debugCamPitch - this.prevDebugCamPitch) * var1;
                GL11.glTranslatef(0.0f, 0.0f, (float)(-var28));
                GL11.glRotatef(var14, 1.0f, 0.0f, 0.0f);
                GL11.glRotatef(var29, 0.0f, 1.0f, 0.0f);
            } else {
                float var29 = var2.rotationYaw;
                float var14 = var2.rotationPitch;
                if (this.mc.gameSettings.thirdPersonView == 2) {
                    var14 += 180.0f;
                }
                double var15 = (double)(-MathHelper.sin(var29 / 180.0f * 1.4545455f * 2.1598449f) * MathHelper.cos(var14 / 180.0f * 15.315265f * 0.20512821f)) * var28;
                double var17 = (double)(MathHelper.cos(var29 / 180.0f * 1.6923077f * 1.8563957f) * MathHelper.cos(var14 / 180.0f * 0.97590363f * 3.219163f)) * var28;
                double var19 = (double)(-MathHelper.sin(var14 / 180.0f * 2.8235295f * 1.1126474f)) * var28;
                for (int var21 = 0; var21 < 8; ++var21) {
                    ...
                    if ((var25 = this.mc.theWorld.rayTraceBlocks(Vec3.createVectorHelper(var5 + (double)(var22 *= 0.1f), var7 + (double)(var23 *= 0.1f), var9 + (double)(var24 *= 0.1f)), Vec3.createVectorHelper(var5 - var15 + (double)var22 + (double)var24, var7 - var19 + (double)var23, var9 - var17 + (double)var24))) == null || !((var26 = var25.hitVec.distanceTo(Vec3.createVectorHelper(var5, var7, var9))) < var28)) continue;
                    var28 = var26;
                }
                if (this.mc.gameSettings.thirdPersonView == 2) {
                    GL11.glRotatef(180.0f, 0.0f, 1.0f, 0.0f);
                }
                GL11.glRotatef(var2.rotationPitch - var14, 1.0f, 0.0f, 0.0f);
                GL11.glRotatef(var2.rotationYaw - var29, 0.0f, 1.0f, 0.0f);
                GL11.glTranslatef(0.0f, 0.0f, (float)(-var28));
                GL11.glRotatef(var29 - var2.rotationYaw, 0.0f, 1.0f, 0.0f);
                GL11.glRotatef(var14 - var2.rotationPitch, 1.0f, 0.0f, 0.0f);
            }
        } else {
        ...
        var5 = var3.prevPosX + (var3.posX - var3.prevPosX) * (double)var1;
        var7 = var3.prevPosY + (var3.posY - var3.prevPosY) * (double)var1 - (double)var4;
        var9 = var3.prevPosZ + (var3.posZ - var3.prevPosZ) * (double)var1;
     ...
     */
    public void setupCameraTransform(float par1, int par2, boolean dragToLookEnabled) {
        ...
        if (this.mc.gameSettings.viewBobbing) {
            if (perspectiveMod.isEnabled()) {
                perspectiveMod.setupViewBobbing(par1, perspectiveMod.bobScreen);
            } else {
        ...
        if (dragToLookEnabled) {
            this.freelookOrientCamera(par1);
        } else {
    ...
    public void renderHand(float par1, int par2, boolean renderItem, boolean renderOverlay, boolean renderTranslucent) {
        if (this.debugViewDirection <= 0) {
            ...
            if (renderItem) {
                ...
                if (this.mc.gameSettings.viewBobbing) {
                    if (perspectiveMod.isEnabled()) {
                        perspectiveMod.setupViewBobbing(par1, perspectiveMod.bobHand);
                    } else {
                        this.setupViewBobbing(par1);
                    }
                }
    ...
    public void updateCameraAndRender(float par1) {
        ...
        if (this.mc.inGameHasFocus && var22) {
            ...
            ModuleDragToLook var9 = CheatBreaker.getInstance().getModuleManager().dragToLook;
            if (!var9.enabled) {
                if (this.mc.gameSettings.smoothCamera) {
                    this.smoothCamYaw += var15;
                    this.smoothCamPitch += var16;
                    float var10 = par1 - this.smoothCamPartialTicks;
                    this.smoothCamPartialTicks = par1;
                    var15 = this.smoothCamFilterX * var10;
                    var16 = this.smoothCamFilterY * var10;
                ...
                var9.setAngles(var15, var16 * (float)var17);
            } else {
                var9.setAngles(var15, var16 * (float)var17);
            }
        }
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    ...
                    var11.addCrashSectionCallable("Mouse location", () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d)", new Object[] {Integer.valueOf(var161), Integer.valueOf(var171), Integer.valueOf(Mouse.getX()), Integer.valueOf(Mouse.getY())}));
                    var11.addCrashSectionCallable("Screen size", () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d", new Object[] {Integer.valueOf(var133.getScaledWidth()), Integer.valueOf(var133.getScaledHeight()), Integer.valueOf(EntityRenderer.this.mc.displayWidth), Integer.valueOf(EntityRenderer.this.mc.displayHeight), Integer.valueOf(var133.getScaleFactor())}));
    ...
    public void renderWorld(float par1, long par2) {
        ...
        float slowChunkLoadingValue = (Float) CheatBreaker.getInstance().getGlobalSettings().slowChunkLoading.getValue();
        ...
        boolean dragToLookEnabled = CheatBreaker.getInstance().getModuleManager().dragToLook.enabled;
        ...
        for (int var13 = 0; var13 < 2; ++var13) {
            ...
            this.setupCameraTransform(par1, var13, dragToLookEnabled);
            ...
            if (this.debugViewDirection == 0) {
                ...
                if (this.mc.objectMouseOver != null && var4.isInsideOfMaterial(Material.water) && var4 instanceof EntityPlayer && !this.mc.gameSettings.hideGUI) {
                    ...
                    if (!this.mc.gameSettings.hideGUI) {
                        if (CheatBreaker.getInstance().getModuleManager().blockOverlayMod.isEnabled()) {
                            CheatBreaker.getInstance().getModuleManager().blockOverlayMod.drawSelectionBox(var21, this.mc.objectMouseOver, 0, par1, this.mc.theWorld);
                        } else {
    ...
    private void addRainParticles() {
        ModuleEnvironmentChanger envChanger = CheatBreaker.getInstance().getModuleManager().environmentChangerMod;
        if (envChanger.isEnabled() && envChanger.customWeather.getBooleanValue() && envChanger.isSnowOrNone()) {
            return;
        }
        ...
        if (var1 != 0.0F && Config.isRainSplash()) {
            ...
            for (int var16 = 0; var16 < var15; ++var16) {
                ...
                if (var19 <= var5 + var7 && var19 >= var5 - var7 && var21.canSpawnLightningBolt() && var21.getFloatTemperature(var17, var19, var18) >= 0.15F || (envChanger.isEnabled() && envChanger.customWeather.getBooleanValue() && envChanger.weatherType.getStringValue().equals("Rain"))) {
    ...
    protected void renderRainSnow(float par1) {
        ModuleEnvironmentChanger envChanger = CheatBreaker.getInstance().getModuleManager().environmentChangerMod;
        boolean allowThroughBlocks = envChanger.allowThroughBlocks.getBooleanValue();
        ...
        if (var411 > 0.0F || envChanger.isEnabled()) {
            this.enableLightmap(par1);

            if (envChanger.isEnabled() && envChanger.customWeather.getBooleanValue() && !envChanger.weatherType.getStringValue().equalsIgnoreCase("None")) {
                var411 = 1.0F;
            }
            ...
            for (int var20 = var471 - var16; var20 <= var471 + var16; ++var20) {
                for (int var21 = var451 - var16; var21 <= var451 + var16; ++var21) {
                    ...
                    if (var25.canSpawnLightningBolt() || var25.getEnableSnow()) {
                        ...
                        if (var27 < var26) {
                            // passing through blocks logic
                            var27 = (envChanger.isEnabled() && allowThroughBlocks ? 0 : var26);
                        }
                        ...
                        if (envChanger.isEnabled() && envChanger.customWeather.getBooleanValue() && envChanger.weatherType.getStringValue().equals("Clear")) {
                            return;
                        }

                        if (var27 != var28 || (envChanger.isEnabled() && envChanger.customWeather.getBooleanValue())) {
                            ...
                            // Else if refused to work, so we got around it!
                            boolean shouldRain;

                            if (envChanger.customWeather.getBooleanValue() && envChanger.isEnabled() && !envChanger.weatherType.getStringValue().equals("Clear"))
                                shouldRain = envChanger.weatherType.getStringValue().equals("Rain");
                            else shouldRain = var441.getWorldChunkManager().getTemperatureAtHeight(var31, var26) >= 0.15F;
                            ...
                            } else if (var441.getWorldChunkManager().getTemperatureAtHeight(var31, var26) < 0.15F || (envChanger.weatherType.getStringValue().equals("Snow") && envChanger.customWeather.getBooleanValue() && envChanger.isEnabled())) {
                                if (var18 != 1) {
                                    if (var18 >= 0) {
                                        var8.draw();
                                    }
