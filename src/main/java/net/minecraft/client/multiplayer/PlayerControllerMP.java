...
import com.cheatbreaker.client.module.impl.normal.hud.simple.combat.SimpleModuleFlickTracker;
import com.cheatbreaker.client.module.impl.normal.hud.simple.combat.SimpleModuleReachDisplay;
...
public class PlayerControllerMP
{
    ...
    public void attackEntity(EntityPlayer playerIn, Entity targetEntity)
    {
        if (playerIn == this.mc.thePlayer) {
            Vec3 distance = this.mc.getRenderViewEntity().getPositionEyes(1.0F);
            SimpleModuleReachDisplay.lastRange = this.mc.objectMouseOver.hitVec.distanceTo(distance);
            SimpleModuleReachDisplay.lastAttackTime = System.currentTimeMillis();

            SimpleModuleFlickTracker flickTrackerModule = CheatBreaker.getInstance().getModuleManager().flickTrackerMod;

            flickTrackerModule.setLastHit(System.currentTimeMillis());
            flickTrackerModule.setLastHitYaw(playerIn.cameraYaw);
            flickTrackerModule.setLastHitPitch(playerIn.cameraPitch);

            flickTrackerModule.setDistance(flickTrackerModule.calculate());
        }
