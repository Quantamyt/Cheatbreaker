...
public abstract class GuiContainer extends GuiScreen
{
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        if (!CheatBreaker.getInstance().getModuleManager().packTweaksMod.isEnabled()
            || (Boolean) CheatBreaker.getInstance().getModuleManager().packTweaksMod.tooltips.getValue()) {
    ...
    {
        CheatBreaker.getInstance().getEventBus().handleEvent(new ClickEvent(state));
        Slot slot = this.getSlotAtPosition(mouseX, mouseY);
        int i = this.guiLeft;
        int j = this.guiTop;
