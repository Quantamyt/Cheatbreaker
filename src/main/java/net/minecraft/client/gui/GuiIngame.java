...
public class GuiIngame extends Gui {
    ...
    private float prevRedScreenBrightness = 1.0F;
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (Config.isVignetteEnabled() || (pt.forceVignette.getValue().equals("Static") && ptIsEnabled)) {
            this.renderVignette(this.mc.thePlayer.getBrightness(partialTicks), scaledresolution);
        } else {
            GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
        }

        if (pt.lowHealthOverlay.getBooleanValue() && pt.isEnabled()) {
            this.renderRedScreen(pt.lowHealthOverlayOpacity.getFloatValue() / 100.0F, scaledresolution);
        } else {
        ...
        if (this.mc.gameSettings.thirdPersonView == 0 && itemstack != null && itemstack.getItem() == Item.getItemFromBlock(Blocks.pumpkin) && showPumpOverlay) {
        ...
        if (this.showCrosshair()) {
            if (CheatBreaker.getInstance().getModuleManager().crosshairMod.isEnabled()) {
                CheatBreaker.getInstance().getModuleManager().crosshairMod.drawCrosshair(xPos / 2, yPos / 2, true);
            } else {
                GlStateManager.tryBlendFuncSeparate(775, 769, 1, 0);
                GlStateManager.enableAlpha();
                this.drawTexturedModalRect(xPos / 2 - 7, yPos / 2 - 7, 0, 0, 16, 16);
            }
        }
        ...
        if (!CheatBreaker.getInstance().getModuleManager().bossBarMod.isRenderHud() && CheatBreaker.getInstance().getModuleManager().bossBarMod.isEnabled()) {
            if (BossStatus.bossName != null && BossStatus.statusBarTime > 0) {
                CheatBreaker.getInstance().getModuleManager().bossBarMod.render(BossStatus.bossName, BossStatus.healthScale, scaledresolution.getScaledWidth() / 2 - 91, 12);
            }
            this.mc.getTextureManager().bindTexture(icons);
        }
        ...
        if (uploadingScreenshot) {
            this.mc.fontRendererObj.drawStringWithShadow("Uploading screenshot...", 4.0f, (float) (scaledresolution.getScaledHeight() - 24), -1);
        }
        if (CheatBreaker.getInstance().getModuleManager().chatMod.hiddenFromHud && this.mc.currentScreen instanceof GuiChat) {
            this.mc.fontRendererObj.drawStringWithShadow("Chat is hidden", 4.0f, (float) (scaledresolution.getScaledHeight() - 24), 0xFF777777);
        }
        ...
        if (this.mc.currentScreen instanceof HudLayoutEditorGui || this.mc.currentScreen instanceof ModulePlaceGui || this.mc.currentScreen instanceof ProfileCreatorGui) {
            CheatBreaker.getInstance().getEventBus().handleEvent(new PreviewDrawEvent(scaledresolution));
        }
        CheatBreaker.getInstance().getEventBus().handleEvent(new GuiDrawBypassDebugEvent(scaledresolution));
        if (!this.mc.gameSettings.showDebugInfo || (Boolean) CheatBreaker.getInstance().getGlobalSettings().showHudInDebug.getValue()) {
            CheatBreaker.getInstance().getEventBus().handleEvent(new GuiDrawEvent(scaledresolution));
        }
        ...
        if (this.recordPlayingUpFor > 0) {
            ...
            if (l1 > 8) {
                ...
                GlStateManager.translate((float) (xPos / 2), (float) (yPos - 68), 0.0F);
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                int l = 16777215;
        ...
        if (this.titlesTimer > 0) {
            ...
            if (i2 > 8) {
                GlStateManager.pushMatrix();
                GlStateManager.translate((float) (xPos / 2), (float) (yPos / 2), 0.0F);
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                GlStateManager.pushMatrix();
                ...
                if (CheatBreaker.getInstance().getGlobalSettings().showServerBasedHitCooldowns.getBooleanValue() && this.displayedSubTitle.contains("˙")) {
        ...
        if (scoreplayerteam != null) {
            ...
            if (i1 >= 0) {
                scoreboard.getObjectiveInDisplaySlot(3 + i1);
            }
        }
        ...
        GlStateManager.disableAlpha();
        GlStateManager.pushMatrix();
        GlStateManager.translate(0.0F, (float) (yPos - 48), 0.0F);
        ...
        if (this.mc.gameSettings.keyBindPlayerList.isKeyDown() && (!this.mc.isIntegratedServerRunning() || this.mc.thePlayer.sendQueue.getPlayerInfoMap().size() > 1 || scoreobjective1 != null)) {
            ...
            this.overlayPlayerList.renderPlayerlist(xPos, scoreboard, scoreobjective1);
        } else {
    ...
    private void renderPlayerStats(ScaledResolution scaledRes) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            for (int i6 = MathHelper.ceiling_float_int((f + f1) / 2.0F) - 1; i6 >= 0; --i6) {
                ...
                if (flag && ModuleOneSevenVisuals.oldHealth.getBooleanValue()) {
                    if (i6 * 2 + 1 < j) {
    ...
    private void renderRedScreen(float opacity, ScaledResolution scaledRes) {
        ModulePackTweaks pt = CheatBreaker.getInstance().getModuleManager().packTweaksMod;
        opacity = this.mc.thePlayer.getHealth() / 2 <= pt.lowHealthStart.getFloatValue() ? opacity : 0.0F;
        opacity = MathHelper.clamp_float(opacity, 0.0F, 1.0F);
        ...
        this.prevRedScreenBrightness = (float) ((double) this.prevRedScreenBrightness + (double) (opacity - this.prevRedScreenBrightness) * 0.01D);
        GlStateManager.disableDepth();
        GlStateManager.depthMask(false);
        ...
        if (f > 0.0F) {
            ...
            GlStateManager.color(0.0F, this.prevRedScreenBrightness, this.prevRedScreenBrightness, 1.0F);
        }
    ...
    private void renderVignette(float lightLevel, ScaledResolution scaledRes) {
        ...
        if (!Config.isVignetteEnabled()) {
            ...
            if (pt.forceVignette.getValue().equals("Amplified") && pt.isEnabled()) {
                ...
                lightLevel *= pt.vignetteOpacityMultiplier.getFloatValue();
            }
