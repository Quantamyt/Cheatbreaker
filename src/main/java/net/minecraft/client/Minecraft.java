...
public class Minecraft implements IPlayerUsage {
    ...
    public ServerData playerServerData; //CheatBreaker
    ...
    public Minecraft(Session p_i1103_1_, int p_i1103_2_, int p_i1103_3_, boolean p_i1103_4_, boolean p_i1103_5_, File p_i1103_6_, File p_i1103_7_, File p_i1103_8_, Proxy p_i1103_9_, String p_i1103_10_, Multimap p_i1103_11_, String p_i1103_12_) {
        ...
        logger.info("Setting UUID: " + p_i1103_1_.getPlayerID());
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        if (var1 != Util.EnumOS.OSX) {
            try {
                InputStream var2 = this.mcDefaultResourcePack.getInputStream(new ResourceLocation("client/icon-1.png"));
                InputStream var3 = this.mcDefaultResourcePack.getInputStream(new ResourceLocation("client/icon-2.png"));
        ...
        this.cbLoadingScreen = new LoadingScreen(114); // CB
        this.cbLoadingScreen.drawMenu(0.0f, 0.0f); // CB
        this.cbLoadingScreen.updatePhase("Sound Handler"); // CB
        this.mcSoundHandler = new SoundHandler(this.mcResourceManager, this.gameSettings);
        this.mcResourceManager.registerReloadListener(this.mcSoundHandler);
        this.mcMusicTicker = new MusicTicker(this);
        this.cbLoadingScreen.updatePhase("Font Renderers"); // CB
        ...
        this.cbLoadingScreen.updatePhase("Items"); // CB
        RenderManager.instance.itemRenderer = new ItemRenderer(this);
        this.cbLoadingScreen.updatePhase("Entities"); // CB
        ...
        this.cbLoadingScreen.updatePhase("World"); // CB
        this.renderGlobal = new RenderGlobal(this);
        this.cbLoadingScreen.updatePhase("Blocks"); // CB
        this.textureMapBlocks = new TextureMap(0, "textures/blocks");
        this.cbLoadingScreen.updatePhase("Anisotropic Filtering"); // CB
        this.textureMapBlocks.func_147632_b(this.gameSettings.anisotropicFiltering);
        this.cbLoadingScreen.updatePhase("Mipmaps"); // CB
        this.textureMapBlocks.func_147633_a(this.gameSettings.mipmapLevels);
        this.cbLoadingScreen.updatePhase("Textures"); // CB
        ...
        this.cbLoadingScreen.updatePhase("Effects"); // CB
        this.effectRenderer = new EffectRenderer(this.theWorld, this.renderEngine);
        this.checkGLError("Post startup");
        this.ingameGUI = new GuiIngame(this);
        new CheatBreaker().initialize(); // CB
        if (this.serverName != null) {
            this.displayGuiScreen(new GuiConnecting(new VanillaMenu(), this, this.serverName, this.serverPort));
            ++MainMenu.menuCounter;
        } else {
            this.displayGuiScreen(new VanillaMenu());
        }

        this.cbLoadingScreen = null;
    ...
    public void scaledTessellator(int p_71392_1_, int p_71392_2_, int p_71392_3_, int p_71392_4_, int p_71392_5_, int p_71392_6_) {
        ...
        var9.addVertexWithUV(p_71392_1_ + 0, p_71392_2_ + p_71392_6_, 0.0D, (float) (p_71392_3_ + 0) * var7, (float) (p_71392_4_ + p_71392_6_) * var8);
        var9.addVertexWithUV(p_71392_1_ + p_71392_5_, p_71392_2_ + p_71392_6_, 0.0D, (float) (p_71392_3_ + p_71392_5_) * var7, (float) (p_71392_4_ + p_71392_6_) * var8);
        var9.addVertexWithUV(p_71392_1_ + p_71392_5_, p_71392_2_ + 0, 0.0D, (float) (p_71392_3_ + p_71392_5_) * var7, (float) (p_71392_4_ + 0) * var8);
        var9.addVertexWithUV(p_71392_1_ + 0, p_71392_2_ + 0, 0.0D, (float) (p_71392_3_ + 0) * var7, (float) (p_71392_4_ + 0) * var8);
        var9.draw();
    }
    ...
    public void displayGuiScreen(GuiScreen p_147108_1_) {
        ...
        if (this.currentScreen != null) {
            if (this.entityRenderer.isShaderActive()) {
                try {
                    if (CheatBreaker.getInstance().getGlobalSettings().guiBlur.getBooleanValue()) {
        ...
        if (p_147108_1_ == null && this.theWorld == null) {
            p_147108_1_ = new VanillaMenu();
        } else if (p_147108_1_ == null && this.thePlayer.getHealth() <= 0.0F) {
            p_147108_1_ = new GuiGameOver();
        }

        if (p_147108_1_ instanceof VanillaMenu) {
            this.gameSettings.showDebugInfo = false;
            if (CheatBreaker.getInstance().getModuleManager().chatMod.isEnabled() && (Boolean) CheatBreaker.getInstance().getModuleManager().chatMod.clearChatOnRelog.getValue()) {
                this.ingameGUI.getChatGUI().func_146231_a();
            }
        }
        if (p_147108_1_ instanceof VanillaMenu) {
            p_147108_1_ = MainMenuBase.MenuTypes.getMenu(CheatBreaker.getInstance().getGlobalSettings().getCurrentMenu());
    ...
    private void runGameLoop() {
        ...
        if (!Display.isActive() && this.fullscreen && (Boolean) CheatBreaker.getInstance().getGlobalSettings().unfullscreenWhenUnfocused.getValue()) {
            this.toggleFullscreen();
        }
        ...
        if (this.isFramerateLimitBelowMax() || (Boolean) CheatBreaker.getInstance().getGlobalSettings().useCustomFPSLimiter.getValue() || (!Display.isActive() && (Boolean) CheatBreaker.getInstance().getGlobalSettings().limitWhenUnfocused.getValue()) || (this.theWorld == null && this.currentScreen != null)) {
    ...
    public void toggleFullscreen() {
        try {
            ...
            if (this.fullscreen) {
                ...
                this.displayWidth = Display.getDisplayMode().getWidth();
                this.displayHeight = Display.getDisplayMode().getHeight();
                if ((Boolean) CheatBreaker.getInstance().getGlobalSettings().borderlessFullscreen.getValue()) {
    ...
    public void runTick() {
        ...
        // CB
        CheatBreaker.getInstance().getEventBus().handleEvent(new TickEvent());
        ...
        if (this.currentScreen == null || this.currentScreen.field_146291_p) {
            ...
            while (Mouse.next()) {
                ...
                if (Mouse.getEventButtonState()) {
                    CheatBreaker.getInstance().getEventBus().handleEvent(new ClickEvent(var9));
                    int n2 = var9 - 100;
                    KeyBinding.onTick(n2);
                    // Lunar Client code. Not sure if it was for a CPS capper? - Tellinq
                    /*if (n2 == this.gameSettings.keyBindAttack.getKeyCode()) {
                        object.add(Mouse.getEventNanoseconds());
                    }*/
                }

                // mice button
                ModuleAutoText autoHotKeyText = CheatBreaker.getInstance().getModuleManager().autoTextMod;
                if (autoHotKeyText.isEnabled()) {
                    for (Setting setting : autoHotKeyText.hotkeys) {
                        if (setting.getSettingName() != null && Mouse.getButtonName(setting.getKeyCode()) != null && Mouse.isButtonDown(setting.getKeyCode())) {
                            if (setting.isHasMouseBind() && setting.getKeyCode() != 0 && var9 == setting.getKeyCode() && autoHotKeyText.messageCount.size() < 5) {
                                Minecraft.getMinecraft().thePlayer.sendChatMessage(setting.getStringValue());
                                autoHotKeyText.messageCount.add(System.currentTimeMillis());
                            } else {
                                autoHotKeyText.messageCount.removeIf(n -> n < System.currentTimeMillis() - 2000L);
                            }
                        }
                    }
                }
                ...
                CheatBreaker.getInstance().getEventBus().handleEvent(new ClickStateEvent(var9, Mouse.getEventButtonState()));
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    ...
                    CheatBreaker.getInstance().getEventBus().handleEvent(new KeyboardEvent(Keyboard.getEventKey()));
                    ...
                    if (autoHotKeyText.isEnabled()) {
                        for (Setting setting : autoHotKeyText.hotkeys) {
                            if (setting.getSettingName() != null && Keyboard.getKeyName(setting.getKeyCode()) != null && Keyboard.getEventKey() == setting.getKeyCode() && autoHotKeyText.messageCount.size() < 5) {
                                Minecraft.getMinecraft().thePlayer.sendChatMessage(setting.getStringValue());
                                autoHotKeyText.messageCount.add(System.currentTimeMillis());
                            } else {
                                autoHotKeyText.messageCount.removeIf(n -> n < System.currentTimeMillis() - 2000L);
...
//                        }
    ...
    private void runTickKeyboard() {
        ...
        while (CheatBreaker.getInstance().getGlobalSettings().keyBindOpenMenu.isPressed()) {
            if (CheatBreaker.getInstance().getGlobalSettings().streamerMode.getBooleanValue() && CheatBreaker.getInstance().getGlobalSettings().disableModMenuKeybind.getBooleanValue()) {
                if (CheatBreaker.getInstance().getGlobalSettings().notifyWhenModMenuKeybindPressed.getBooleanValue()) {
                    String instruction = CheatBreaker.getInstance().getGlobalSettings().holdDownModsGameMenuButton.getBooleanValue() ? "hold " + EnumChatFormatting.GOLD + "Mods" + EnumChatFormatting.RESET + " for " + EnumChatFormatting.AQUA + CheatBreaker.getInstance().getGlobalSettings().holdDuration.getFloatValue() + " seconds" : "click " + EnumChatFormatting.GOLD + "Mods";
                    CheatBreaker.getInstance().getModuleManager().notificationsMod.send("info", "Streamer mode is " + EnumChatFormatting.GREEN + "enabled" + EnumChatFormatting.RESET + "! Press escape and " + instruction + EnumChatFormatting.RESET + " to open the Mod Menu.", 5000L);
                }
            } else {
                this.displayGuiScreen(new HudLayoutEditorGui());
            }
        }

//        while (CheatBreaker.getInstance().getGlobalSettings().keyBindOpenVoiceMenu.isPressed()) {
//            this.displayGuiScreen(new VoiceChatGui());
//        }

        while (CheatBreaker.getInstance().getGlobalSettings().keyBindDragToLook.isPressed()) {
            CheatBreaker.getInstance().getModuleManager().dragToLook.toggle();
        }

        while (CheatBreaker.getInstance().getGlobalSettings().keyBindBackLook.isPressed()) {
            CheatBreaker.getInstance().getModuleManager().snapLook.toggle(1);
        }

        while (CheatBreaker.getInstance().getGlobalSettings().keyBindFrontLook.isPressed()) {
            CheatBreaker.getInstance().getModuleManager().snapLook.toggle(2);
        }
    }
    ...
    public void loadWorld(WorldClient p_71353_1_, String p_71353_2_) {
        ...
        if (p_71353_1_ != null) {
            CheatBreaker.getInstance().getEventBus().handleEvent(new LoadWorldEvent(p_71353_1_));
            CheatBreaker.getInstance().getCbNetHandler().getNametagsMap().clear();
            if (this.renderGlobal != null) {
        ...
        System.gc();
        this.systemTime = 0L;
        CheatBreaker.getInstance().getEventBus().handleEvent(new ConnectEvent());
    }
    ...
    public void setServerData(ServerData p_71351_1_) {
        this.playerServerData = this.currentServerData;
...
//                        this.ingameGUI.getChatGUI().func_146227_a(ScreenShotHelper.saveScreenshot(this.mcDataDir, this.displayWidth, this.displayHeight, this.mcFrameBuffer));
                        ScreenShotHelper.saveScreenshot(this.mcDataDir, this.displayWidth, this.displayHeight, this.mcFrameBuffer);
                    }
                }
            }
        }
