...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    private void  startGame() throws LWJGLException, IOException {
        ...
        this.cbLoadingScreen = new LoadingScreen(115); // CB
        this.cbLoadingScreen.drawMenu(0.0f, 0.0f); // CB
        this.cbLoadingScreen.updatePhase("Skins"); // CB
        this.initStream();
        ...
        this.cbLoadingScreen.updatePhase("Achievements"); // CB
    ...
    private void setWindowIcon() {
        ...
        if (util$enumos != Util.EnumOS.OSX) {
            ...
            try {
                inputstream = this.mcDefaultResourcePack.getInputStream(new ResourceLocation("client/icon-1.png"));
                inputstream1 = this.mcDefaultResourcePack.getInputStream(new ResourceLocation("client/icon-2.png"));
    ...
    public void displayGuiScreen(GuiScreen guiScreenIn) {
        CheatBreaker.getInstance().lastScreen = this.currentScreen;
        ...
        if (guiScreenIn instanceof GuiMainMenu) {
            guiScreenIn = MainMenuBase.MenuTypes.getMenu(CheatBreaker.getInstance().getGlobalSettings().getCurrentMenu());
    ...
    private void runGameLoop() throws IOException {
        ...
        /*
         * 1.7.10 feature - This is now toggleable in the case someone wants to have this back.
         * Feature: Toggles fullscreen (off) when the game is unfocused and is in fullscreen.
         */
        if (!Display.isActive() && this.fullscreen && CheatBreaker.getInstance().getGlobalSettings().unfullscreenWhenUnfocused.getBooleanValue()) {
            this.toggleFullscreen();
        }
    ...
    public void toggleFullscreen() {
        try {
            ...
            if (this.fullscreen) {
                ...
                if (CheatBreaker.getInstance().getGlobalSettings().borderlessFullscreen.getBooleanValue()) {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            while (Mouse.next()) {
                ...
                if (Mouse.getEventButtonState()) {
                    if (this.thePlayer.isSpectator() && i == 2) {
                        ...
                        CheatBreaker.getInstance().getEventBus().handleEvent(new ClickEvent(i));
                    }
                }
                ...
                if (autoHotKeyText.isEnabled()) {
                    for (Setting setting : autoHotKeyText.hotkeys) {
                        if (setting.getSettingName() != null && Mouse.getButtonName(setting.getKeyCode()) != null && Mouse.isButtonDown(setting.getKeyCode())) {
                            if (setting.isHasMouseBind() && setting.getKeyCode() != 0 && i == setting.getKeyCode() && autoHotKeyText.messageCount.size() < autoHotKeyText.maxMessagesPerInterval) {
                                autoHotKeyText.send(setting.getStringValue());
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    ...
                    // For some reason the event didn't like it when I did it in the actual mod class without being disabled and enabled again, so I did it here. - Nox
                    if (Keyboard.getEventKey() == CheatBreaker.getInstance().getModuleManager().nameTagMod.hideNamePlatesKeybind.getIntegerValue()) {
                    ...
                    if (autoHotKeyText.isEnabled()) {
                        for (Setting setting : autoHotKeyText.hotkeys) {
                            if (setting.getSettingName() != null && Keyboard.getKeyName(setting.getKeyCode()) != null && Keyboard.getEventKey() == setting.getKeyCode() && autoHotKeyText.messageCount.size() < autoHotKeyText.maxMessagesPerInterval) {
                                autoHotKeyText.send(setting.getStringValue());
                                autoHotKeyText.messageCount.add(System.currentTimeMillis());
                            } else {
                                autoHotKeyText.messageCount.removeIf(n -> n < System.currentTimeMillis() - 2000L);
...
//                        }
    ...
    private void runTickKeyboard() {
        while (this.gameSettings.keyBindTogglePerspective.isPressed()) {
            ...
            if (!CheatBreaker.getInstance().getModuleManager().motionBlurMod.isEnabled()) {
                if (this.gameSettings.thirdPersonView == 0) {
                    this.entityRenderer.loadEntityShader(this.getRenderViewEntity());
                } else if (this.gameSettings.thirdPersonView == 1) {
        ...
        while (CheatBreaker.getInstance().getGlobalSettings().keyBindDragToLook.isPressed()) {
            if (this.getCurrentServerData() != null
                    && CheatBreaker.getInstance().getModuleManager().disallowedModManager.disallows(
                    this.getCurrentServerData().serverIP.toLowerCase(),
                    "freelook"
            ) &&
                    !this.isIntegratedServerRunning()) {
                CheatBreaker.getInstance().getModuleManager().notificationsMod.send("error", "Drag to Look is " + EnumChatFormatting.RED + "disallowed" + EnumChatFormatting.RESET + " on Hypixel.", 3000L);
            } else {
    ...
    public void loadWorld(WorldClient worldClientIn, String loadingMessage) {
        ...
        if (worldClientIn != null) {
            CheatBreaker.getInstance().getEventBus().handleEvent(new LoadWorldEvent(worldClientIn));
            CheatBreaker.getInstance().getCBNetHandler().getCustomNameTags().clear();
            if (this.renderGlobal != null) {
